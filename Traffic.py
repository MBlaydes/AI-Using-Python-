...
def load_data(data_dir):
    ...
    # Iterate through images in each folder
        for filename in os.listdir(os.path.join(data_dir, foldername)):
            ...
            # Normalise image pixel intensities, it scales the data to a consistent range (typically 0-1)
            img = img/255
...
def get_model():
    ...
    # Create a convolutional neural network with kernel matrix with RBG channel value using keras
    model = tf.keras.models.Sequential([
        ...
        # Add a hidden layer with 128 units and half rate dropout (increased units during testing)
        tf.keras.layers.Dense(450, activation="relu"),
        tf.keras.layers.Dropout(0.5),

        # Add an output layer with output units for NUM_CAT units and softmax probability distribution
